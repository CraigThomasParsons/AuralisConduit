...
def dispatch_package(sender_id: str, recipient_id: str, package_id: str, outbox_dir: str):
    ...
    rabbitmq_user = os.environ.get("POSTAL_RABBITMQ_USER", "postalWorker")
    rabbitmq_password = os.environ.get("POSTAL_RABBITMQ_PASSWORD", "")
    rabbitmq_host = os.environ.get("POSTAL_RABBITMQ_HOST", "localhost")
    if not rabbitmq_password:
        print(f"[PostalService] ERROR: POSTAL_RABBITMQ_PASSWORD is not configured. Cannot deliver {package_id}")
        return
